#' @title Read and print the output from 'extended_summary'
#'
#' @description \code{extended_summary} reads and prints the saved output from 'extended_summary'.
#' @usage read_summary(path = ".", fill = "#6DCD59FF", highlight = FALSE)
#' @param path Directory in which to look for the 'extended_summary' output files.
...
#' @return If \code{plot = TRUE} the function reads the saved output of \code{\link{extended_summary}} and prints it.
#' @export
#' @name read_summary
#' @details It prints a summary of brmsfit results that were previously saved by \code{\link{extended_summary}}. Useful when models are too big so reading them many times is to be avoided. It includes a model fit table, a coefficient table and a posterior distribution halfeye graph next to a chain trace plot. Tables are produce in html format so the output is nicely printed when knit in Rmarkdown files. You might have to add 'results = 'as.is', warning = FALSE and adjust fig.width to chunk options in Rmarkdown documents.
#' @examples
#' {
#' # run model
...
#' }
#' @seealso \code{\link{combine_rds_fits}}, \code{\link{extended_summary}}
