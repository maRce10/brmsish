brms_model_summary <- function(model = NULL, gsub.pattern = NULL, gsub.replacement = NULL, xlab = "Effect size", n.subposts = 2000, model_name = NULL, read.file = NULL, plot.area.prop = 0.9, remove.intercepts = FALSE, fill = "#6DCD59FF"){
  ...
  if (remove.intercepts)
    summ <- summ[grep("^Intercept", rownames(summ), invert = TRUE), ]
  ...
  coef_table <- coef_table[, c("Estimate", "Rhat", "Bulk_ESS", "l.95..CI", "u.95..CI")]
  ...
  model_table$`rhats > 1.05` <- sum(stats::na.omit(brms::rhat(model)) > 1.05)
  ...
  if(!is.null(gsub.pattern) & !is.null(gsub.replacement)){
    ...
    for(i in 1:length(gsub.pattern)){
      posteriors_by_chain$variable <- gsub(pattern = gsub.pattern[i], replacement = gsub.replacement[i], posteriors_by_chain$variable)
      posteriors$variable <- gsub(pattern = gsub.pattern[i], replacement = gsub.replacement[i], posteriors$variable)
      coef_table2$variable <- gsub(pattern = gsub.pattern[i], replacement = gsub.replacement[i], coef_table2$variable)
  ...
  posteriors_by_chain$variable <- factor(posteriors_by_chain$variable, levels = sort(unique(posteriors_by_chain$variable), decreasing = TRUE))

  col_pointrange <- if(all(coef_table2$significance == "non-sig")) "gray" else
    if(all(coef_table2$significance == "sig")) "black" else
      c("gray", "black")

  posteriors$significance <- sapply(posteriors$variable, function(x) coef_table2$significance[coef_table2$variable == x])

  gg_dists <- ggplot2::ggplot(data = posteriors, ggplot2::aes(y = variable, x = value, fill = significance)) +
  ...
  gg_traces <- ggplot2::ggplot(data = posteriors_by_chain, ggplot2::aes(x = iteration, y = value, color = chain)) +
    ggplot2::geom_line() +
    ggplot2::scale_color_viridis_d(alpha = 0.7, begin = 0.2, end = 0.9) +
    ggplot2::facet_wrap(~ variable, scales = "free_y", ncol = 1,
               strip.position = "right") +
    ggplot2::theme_classic() +
  ...
  df2 <- row_spec(kable_input = df2, row =  which(coef_table$CI_low * coef_table$CI_high > 0), background = grDevices::adjustcolor(fill, alpha.f = 0.3))
